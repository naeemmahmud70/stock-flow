...
import { Modal, PriorityBadge, Alert, EmptyState, Spinner } from "@/components/ui";
import { cn } from "@/lib/utils";
import { RestockItem, Product } from "@/types";
import { RefreshCw, PackagePlus, CheckCircle } from "lucide-react";

function RestockModal({ item, onClose, onSuccess }: { item: RestockItem; onClose: () => void; onSuccess: () => void }) {
  ...
  const handleRestock = async () => {
    if (addStock < 1) { setError("Enter a valid quantity"); return; }
    setLoading(true);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-slate-50 rounded-xl p-4 space-y-2">
        <div className="flex justify-between text-xs">
          ...
          <span className="font-semibold text-red-600">{item.currentStock} units</span>
        </div>
        <div className="flex justify-between text-xs">
          <span className="text-slate-500">Minimum threshold</span>
          <span className="font-medium text-slate-700">{item.threshold} units</span>
        </div>
        <div className="flex justify-between text-xs">
          <span className="text-slate-500">Suggested restock</span>
          <span className="font-medium text-blue-600">{item.threshold * 2} units</span>
        </div>
        <div className="w-full bg-slate-200 rounded-full h-1.5 mt-2">
          <div
            className="bg-red-400 h-1.5 rounded-full"
            style={{ width: `${Math.min(100, (item.currentStock / item.threshold) * 100)}%` }}
          />
        </div>
      </div>
      ...
      <div className="flex gap-3 justify-end pt-1">
        ...
        <button onClick={handleRestock} disabled={loading} className="sf-btn-primary">
          {loading ? <Spinner size="sm" /> : <PackagePlus className="w-4 h-4" />}
          Restock
        </button>
      </div>
    </div>
...
export default function RestockPage() {
  ...
  const sorted = [...items].sort((a, b) =>
    priorityOrder[a.priority] - priorityOrder[b.priority] || a.currentStock - b.currentStock
  );
  ...
  const ItemRow = ({ item }: { item: RestockItem }) => {
    const product = item.product as Product;
    const pct = Math.min(100, Math.round((item.currentStock / item.threshold) * 100));
    return (
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1 bg-slate-100 rounded-full h-1.5">
              <div
                className={cn("h-1.5 rounded-full transition-all", item.priority === "high" ? "bg-red-400" : item.priority === "medium" ? "bg-amber-400" : "bg-blue-400")}
                style={{ width: `${pct}%` }}
              />
            </div>
  ...
  const Section = ({ title, items, color }: { title: string; items: RestockItem[]; color: string }) => {
    if (items.length === 0) return null;
    return (
      <div className="sf-card overflow-hidden">
        <div className={cn("px-5 py-3 border-b border-slate-100 flex items-center gap-2", color)}>
          <span className="text-xs font-semibold uppercase tracking-wide">{title}</span>
          <span className="ml-auto text-xs font-medium bg-white/60 px-2 py-0.5 rounded-full">{items.length}</span>
        </div>
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-slate-800">Restock Queue</h2>
          <p className="text-xs text-slate-400">{items.length} product{items.length !== 1 ? "s" : ""} need restocking</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          <Section title="🔴 High priority" items={high} color="text-red-600 bg-red-50" />
          <Section title="🟡 Medium priority" items={medium} color="text-amber-600 bg-amber-50" />
          <Section title="🔵 Low priority" items={low} color="text-blue-600 bg-blue-50" />
        </div>
      )}
      ...
      >
        {restocking && (
          <RestockModal
            item={restocking}
            onClose={() => setRestocking(null)}
            onSuccess={() => handleSuccess((restocking.product as Product)?.name)}
          />
        )}
      </Modal>
    </div>
